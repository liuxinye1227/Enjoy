...
CONF_DIR=${root}/conf
LIB_DIR=${root}/lib
PID_FILE=${root}/fs-wechat-access.pid
LOG_DIR=${root}/logs
STDOUT_FILE=${LOG_DIR}/stdout.log
MAIN_CLASS=com.facishare.wechat.access.AccessServer
...
function do_start()
{
    ...
    ${JAVA} ${OPT} ${HEAP} ${GC} ${GC_LOG}  -cp ${CLASSPATH}:${CONF_DIR}  ${MAIN_CLASS} > ${STDOUT_FILE} 2>&1 &
